#!/usr/bin/env node
/**
 * Factor E1-PRI SIP settings pages (SipSipPage, SipMediaPage).
 * Preserves logic; splits into Page + hooks + utils + components.
 */
import fs from "fs";
...
function readLines(file) {
  return fs.readFileSync(path.join(SIP, file), "utf8").split(/\r?\n/);
}
...
// ── SipSipPage ──────────────────────────────────────────────
{
  ...
  console.log("SipSipPage lines:", lines.length);

  // Validators
  write(
    "utils/SipSipValidators.js",
