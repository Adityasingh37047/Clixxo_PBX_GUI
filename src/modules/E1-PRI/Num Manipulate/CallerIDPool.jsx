...
const CallerIDPool = () => {
  ...
  const handleInverse = (table) => {
    if (table === "ip_pstn") {
      setCheckedIpPstn(
        rowsIpPstn
          .map((_, idx) => (checkedIpPstn.includes(idx) ? null : idx))
          .filter((i) => i !== null),
      );
    } else {
      setCheckedPstnIp(
        rowsPstnIp
          .map((_, idx) => (checkedPstnIp.includes(idx) ? null : idx))
          .filter((i) => i !== null),
      );
    }
  };
                  ...
                  <tr key={idx} style={{ borderBottom: "1px solid #bbb" }}>
                    <td className="border border-gray-400 px-1 py-0.5 text-center bg-white">
                      <input
                        ...
                        onChange={() => handleCheck(tableKey, idx)}
                        className="w-4 h-4"
                      />
                    </td>
                    ...
                    <td className="border border-gray-400 px-1 py-0.5 text-center bg-white">
                      <EditDocumentIcon
                        ...
                        className="w-5 h-5"
                        onClick={() => handleEdit(tableKey, idx)}
                      />
                    </td>
                  </tr>
                ))}
  ...
  return (
    ...
    >
      ...
      <form
        onSubmit={handleSet}
        className=" rounded-lg p-0 md:p-6 mb-0 border-none w-full mx-auto"
      >
        ...
        <div className="text-red-600 font-medium text-sm text-center w-full">
          Note: IP-&gt;PSTN Outbound Calls with Designated CallerID set to 0
          means the feature is disabled; Usage Cycle set to 0 means not to clear
          counts.
        </div>
      </form>
      {/* Dual tables */}
      ...
      >
        <DialogTitle className="bg-gray-600 text-white text-center font-semibold text-lg">
          CallerID
        </DialogTitle>
