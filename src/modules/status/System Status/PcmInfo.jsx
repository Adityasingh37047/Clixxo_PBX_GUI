...
const PcmInfo = () => {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto font-sans text-sm p-0" style={{backgroundColor: "var(--bg-muted)"}}>
      {/* PCM0 Dropdown above the blue bar */}
      <div className="flex justify-end mb-1">
        <select className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] px-4 py-1 text-xs" style={{ backgroundColor: 'var(--bg-surface)' }}>
          <option>PCM0</option>
        </select>
      </div>

      {/* Blue bar header OUTSIDE the border */}
      <div className="w-full h-8 bg-gradient-to-b from-[#d0ecff] via-[#7ecbfa] to-[#3b8fd6] flex items-center justify-center font-semibold text-base text-[var(--text-secondary)] shadow mb-0">
        <span>PCM Info</span>
      </div>
          ...
          <div className="space-y-3">
            {PCM_FIELDS.map((field) => (
              <div key={field.key} className="flex flex-col md:flex-row items-center justify-between gap-1 md:gap-0">
                <label className="text-[var(--text-secondary)] md:w-80 w-full mb-1 md:mb-0">{field.label}</label>
                <input
                  type="text"
                  value={formData[field.key]}
                  onChange={(e) => handleInputChange(field.key, e.target.value)}
                  onFocus={() => setCurrentField(field.key)}
                  className={`w-full md:w-60 px-2 py-1 border border-[var(--border-subtle)] bg-[var(--bg-surface)] text-center rounded ${
