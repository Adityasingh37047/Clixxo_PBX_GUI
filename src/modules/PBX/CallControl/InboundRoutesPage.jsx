...
const Btn = ({ children, onClick, disabled, variant = "default", style: extraStyle }) => {
  ...
  return (
    <button onClick={onClick} disabled={disabled}
      style={{ ...s, fontSize: 11, fontWeight: 600, padding: "5px 14px", borderRadius: 6, cursor: disabled ? "not-allowed" : "pointer", opacity: disabled ? 0.5 : 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 5, transition: "opacity 0.15s ease", whiteSpace: "nowrap", ...extraStyle }}
...
const FieldRow = ({ label, children }) => (
  <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
    <label style={{ fontSize: 13, fontWeight: 600, color: C.labelText, width: 170, flexShrink: 0 }}>{label}</label>
...
const InboundRoutesPage = () => {
  ...
  const moveTrunkUp = () => {
    ...
    setSelectedTrunks((prev) => {
      ...
      for (let i = 1; i < arr.length; i++) {
        if (chosenSelected.includes(arr[i]) && !chosenSelected.includes(arr[i - 1]))
  ...
  const inputStyle = { width: "100%", border: `1px solid ${C.cardBorder}`, borderRadius: 4, padding: "6px 8px", fontSize: 13, outline: "none", color: C.valueText, background: "#fff", boxSizing: "border-box" };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      showMessage("success", editId != null ? "Inbound route updated successfully." : "Inbound route created successfully.");
      await fetchInboundRoutes();
      handleCloseModal();
    } catch (err) {
  ...
  return (
    <div style={{ backgroundColor: C.pageBg, minHeight: "calc(100vh - 80px)", padding: 16 }}>
      <div style={{ maxWidth: "100%", margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: 12 }}>
          <span style={{ fontSize: 11, color: C.mutedText }}>
            ...
            <span style={{ color: "#1e293b", fontWeight: 600 }}>Inbound Routes</span>
          </span>
        </div>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "auto", minWidth: 900 }}>
                <thead>
                  <tr>
                    ...
                    <TH>Destination</TH>
                    <TH>Enabled</TH>
                    <TH style={{ textAlign: "left", paddingLeft: 16 }}>Member Trunks</TH>
      ...
      {/* Import Modal */}
      <Dialog open={showImportModal} onClose={() => { if (!importLoading) { setShowImportModal(false); setImportFile(null); } }}
        maxWidth={false} PaperProps={{ sx: { width: 420, maxWidth: "96vw", mx: "auto", p: 0 } }}>
        ...
        <DialogContent style={{ backgroundColor: C.pageBg, padding: "20px 24px 12px" }}>
          <div className="flex flex-col gap-4 pt-1">
            ...
            <div className="border-2 border-dashed border-gray-400 rounded-lg p-6 text-center cursor-pointer hover:border-[#7B8FA8] hover:bg-[#EEF2F7] transition-colors" onClick={() => importFileRef.current?.click()}>
              <div className="text-gray-500 text-[13px] mb-1">
                {importFile ? <span className="text-green-700 font-semibold">{importFile.name}</span> : <span>Click to choose file <span className="text-gray-400">(CSV / JSON)</span></span>}
              </div>
        ...
        <DialogActions style={{ backgroundColor: C.pageBg, justifyContent: "center", gap: 16, padding: "12px 24px 16px" }}>
          <Btn onClick={handleImportSubmit} disabled={importLoading || !importFile}>{importLoading ? "Importing..." : "Import"}</Btn>
      ...
      >
        <DialogTitle style={{ background: "#1e2d42", color: "#fff", fontWeight: 700, fontSize: 16, textAlign: "center", padding: "14px 24px" }}>
          {editId != null ? "Edit Inbound Route" : "Add Inbound Route"}
        </DialogTitle>
        <DialogContent style={{ padding: "20px 24px", backgroundColor: C.pageBg }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {/* Route Settings */}
            <div style={{ background: "#fff", border: `1px solid ${C.cardBorder}`, borderRadius: 6, padding: 16 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: C.labelText, marginBottom: 14, borderBottom: `1px solid ${C.cardBorder}`, paddingBottom: 6 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px 32px" }}>
                ...
                <FieldRow label="Enabled">
                  <FormControl size="small" fullWidth>
                    <Select value={enabled} onChange={(e) => setEnabled(e.target.value)} sx={{ fontSize: 13 }}>
                      {ENABLE_OPTIONS.map((opt) => <MenuItem key={opt} value={opt} sx={{ fontSize: 13 }}>{opt}</MenuItem>)}
                    </Select>
                  </FormControl>
                </FieldRow>

                <FieldRow label="DID Pattern">
                  <input style={inputStyle} value={didPattern} onChange={(e) => setDidPattern(e.target.value)} />
                </FieldRow>
                <FieldRow label="Priority">
                  <input style={inputStyle} value={priority} onChange={(e) => setPriority(e.target.value)} />
                </FieldRow>

                <FieldRow label="Caller ID Pattern">
                  <input style={inputStyle} value={callerIdPattern} onChange={(e) => setCallerIdPattern(e.target.value)} />
                </FieldRow>
                <FieldRow label="Enable Mobility Extension">
                  <FormControl size="small" fullWidth>
                    <Select value={enableMobilityExtension} onChange={(e) => setEnableMobilityExtension(e.target.value)} sx={{ fontSize: 13 }}>
                      {MOBILITY_OPTIONS.map((opt) => <MenuItem key={opt} value={opt} sx={{ fontSize: 13 }}>{opt}</MenuItem>)}
                    </Select>
                  </FormControl>
                </FieldRow>

                <FieldRow label="Distinctive RingTone">
                  <input style={inputStyle} value={distinctiveRingTone} onChange={(e) => setDistinctiveRingTone(e.target.value)} />
                </FieldRow>
                <FieldRow label="Send RingTone">
                  <FormControl size="small" fullWidth>
                    <Select value={sendRingTone} onChange={(e) => setSendRingTone(e.target.value)} sx={{ fontSize: 13 }}>
                      {SEND_RINGTONE_OPTIONS.map((opt) => <MenuItem key={opt} value={opt} sx={{ fontSize: 13 }}>{opt}</MenuItem>)}
                    </Select>
                  </FormControl>
                </FieldRow>
                ...
                <FieldRow label="Enable T.38">
                  <FormControl size="small" fullWidth>
                    <Select value={enableT38} onChange={(e) => setEnableT38(e.target.value)} sx={{ fontSize: 13 }}>
                      {T38_OPTIONS.map((opt) => <MenuItem key={opt} value={opt} sx={{ fontSize: 13 }}>{opt}</MenuItem>)}
                    </Select>
                  </FormControl>
                </FieldRow>
                <FieldRow label="Enable Time Condition">
                  <FormControl size="small" fullWidth>
                    <Select value={enableTimeCondition} onChange={(e) => setEnableTimeCondition(e.target.value)} sx={{ fontSize: 13 }}>
                      {TIME_CONDITION_OPTIONS.map((opt) => <MenuItem key={opt} value={opt} sx={{ fontSize: 13 }}>{opt}</MenuItem>)}
                    </Select>
                  </FormControl>
                </FieldRow>

                <div style={{ gridColumn: "1 / -1", borderTop: `1px solid ${C.cardBorder}`, margin: "4px 0" }} />

                <FieldRow label="Destination *">
                  <FormControl size="small" fullWidth>
                    <Select value={destination} onChange={(e) => { setDestination(e.target.value); setDestinationTarget(""); setExtensionRange(""); }}
                      displayEmpty MenuProps={SELECT_MENU_PROPS} sx={{ fontSize: 13 }}>
                      <MenuItem value=""><em>Select</em></MenuItem>
                      {DESTINATION_OPTIONS.map((opt) => <MenuItem key={opt} value={opt} sx={{ fontSize: 13 }}>{opt}</MenuItem>)}
                    </Select>
                  </FormControl>
                </FieldRow>
                ...
                {destination === "Extension_Range" ? (
                  <FieldRow label="Extension Range *">
                    <input style={inputStyle} value={extensionRange} onChange={(e) => setExtensionRange(e.target.value)} placeholder="100-136" />
                  </FieldRow>
                ) : needsDestinationTarget ? (
                  <FieldRow label="Destination Value *">
                    <FormControl size="small" fullWidth>
                      <Select value={destinationTarget} onChange={(e) => setDestinationTarget(e.target.value)} displayEmpty MenuProps={SELECT_MENU_PROPS} sx={{ fontSize: 13 }}>
                        <MenuItem value="" disabled={destinationChoices.length === 0}><em>Select</em></MenuItem>
                        {destinationChoices.length === 0
                          ? <MenuItem value="" disabled sx={{ fontSize: 13 }}>No options available</MenuItem>
                          : destinationChoices.map((opt) => <MenuItem key={opt.id} value={opt.id} sx={{ fontSize: 13 }}>{opt.label}</MenuItem>)}
                      </Select>
                    </FormControl>
                  </FieldRow>
                ) : <div />}
            ...
            <div style={{ background: "#fff", border: `1px solid ${C.cardBorder}`, borderRadius: 6, padding: 16 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 48px 1fr 48px", gap: 12 }}>
                <div>
                  <div style={{ fontSize: 12, fontWeight: 600, color: "#325a84", textAlign: "center", marginBottom: 8 }}>Available</div>
                  <select multiple value={availableSelected}
                    onChange={(e) => setAvailableSelected(Array.from(e.target.selectedOptions, (opt) => opt.value))}
                    style={{ width: "100%", height: 160, border: `1px solid ${C.cardBorder}`, background: "#fff", borderRadius: 4, padding: "4px 8px", fontSize: 13, outline: "none" }}>
                    {loading.trunks ? <option>Loading trunks...</option>
                      : availableList.length === 0 ? <option disabled>No trunks</option>
                      : availableList.map((t) => <option key={t.id} value={t.id}>{t.label}</option>)}
                  </select>
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 4, paddingTop: 28 }}>
                  <button type="button" className="h-9 border border-gray-500 bg-[#d9dde3] text-sm font-semibold hover:bg-[#c5cbd3]" onClick={addSelectedTrunks}>&gt;</button>
                  <button type="button" className="h-9 border border-gray-500 bg-[#d9dde3] text-sm font-semibold hover:bg-[#c5cbd3]" onClick={addAllTrunks}>&gt;&gt;</button>
                  <button type="button" className="h-9 border border-gray-500 bg-[#d9dde3] text-sm font-semibold hover:bg-[#c5cbd3]" onClick={removeSelectedTrunks}>&lt;</button>
                  <button type="button" className="h-9 border border-gray-500 bg-[#d9dde3] text-sm font-semibold hover:bg-[#c5cbd3]" onClick={removeAllTrunks}>&lt;&lt;</button>
                </div>
                <div>
                  <div style={{ fontSize: 12, fontWeight: 600, color: "#325a84", textAlign: "center", marginBottom: 8 }}>Selected</div>
                  <select multiple value={chosenSelected}
                    onChange={(e) => setChosenSelected(Array.from(e.target.selectedOptions, (opt) => opt.value))}
                    style={{ width: "100%", height: 160, border: `1px solid ${C.cardBorder}`, background: "#fff", borderRadius: 4, padding: "4px 8px", fontSize: 13, outline: "none" }}>
                    {selectedTrunks.length === 0 ? <option disabled>No selected trunks</option>
                      : selectedTrunks.map((id) => <option key={id} value={id}>{getTrunkLabel(id)}</option>)}
                  </select>
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 4, paddingTop: 28 }}>
        ...
        <DialogActions style={{ padding: "16px 24px", justifyContent: "center", gap: 16 }}>
          <Btn onClick={handleSave} disabled={loading.save} style={{ minWidth: 110, padding: "8px 24px", fontSize: 13 }}>
            {loading.save ? <><CircularProgress size={14} style={{ color: "#fff" }} /> Saving...</> : "Save"}
          </Btn>
          <Btn onClick={handleCloseModal} disabled={loading.save} variant="outline" style={{ minWidth: 110, padding: "8px 24px", fontSize: 13 }}>
            Close
          </Btn>
        </DialogActions>
      </Dialog>
