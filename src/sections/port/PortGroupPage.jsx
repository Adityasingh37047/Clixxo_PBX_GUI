...
const PortGroupPage = () => {
  ...
  const handlePortToggle = (idx) => {
    setForm((prev) => {
      const nextPorts = prev.ports.map((v, i) => (i === idx ? !v : v));
      if (nextPorts.some(Boolean) && validationErrors.ports) {
        setValidationErrors((prevErrs) => ({ ...prevErrs, ports: "" }));
      }
      return {
        ...prev,
        ports: nextPorts,
  ...
  const handleInversePorts = () => {
    setForm((prev) => {
      const nextPorts = prev.ports.map((v) => !v);
      if (nextPorts.some(Boolean) && validationErrors.ports) {
        setValidationErrors((prevErrs) => ({ ...prevErrs, ports: "" }));
      }
      return {
  ...
  const handleSave = (e) => {
    ...
    if (!checkAnyPortSelected()) {
      errs.ports = "Please choose at least one port!";
    }
    ...
    if (form.id) {
      setGroups((prev) => prev.map((g) => (g.id === form.id ? newGroup : g)));
      showMessage("success", "Port group updated successfully!");
    } else {
      setGroups((prev) => [...prev, newGroup]);
      showMessage("success", "Port group created successfully!");
    }
  ...
  const handleClearAllFieldsAndPorts = () => {
    setForm(initialFormState());
    setPortsPage(1);
    setValidationErrors({});
  ...
  const handleDeleteGroup = () => {
    if (form.id) {
      ...
      showMessage("success", "Port group deleted successfully!");
    }
    setShowAddForm(false);
  };
            ...
            <thead>
              <tr>
                {PORT_GROUP_TABLE_COLUMNS.map((col) => {
                  if (col.key === "check") {
                    return (
                      <TH key={col.key} style={{ width: 36 }}>
                        <Checkbox
                          size="small"
                          checked={
                            groups.length > 0 &&
                            groups.every((g) => checkedRows[g.id])
                          }
                          indeterminate={
                            groups.some((g) => checkedRows[g.id]) &&
                            !groups.every((g) => checkedRows[g.id])
                          }
                          onChange={handleTableCheckAll}
                          sx={{
                            padding: "1px",
                            color: C.accent,
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <Btn
                        onClick={() => {
                          const portSelectVal = PORT_GROUP_SELECT_MODE_OPTIONS.find(
                            (o) => o.label === group.portSelectMode
                          )?.value || "0";
                          const isRegistered = group.sipAccount !== "---" ? "1" : "0";
                          setForm({
                            id: group.id,
                            index: group.index,
                            description: group.description,
                            registerPortGroup: isRegistered,
                            sipAccount: group.sipAccount === "---" ? "" : group.sipAccount,
                            displayName: group.displayName === "---" ? "" : group.displayName,
                            password: group.password || "",
                            authUserName: group.authUserName || "",
                            registerSelectMode: group.registerSelectMode || "0",
                            portSelectMode: portSelectVal,
                            enumRule: group.enumRule === "---" ? "" : group.enumRule,
                            ringExpire: group.ringExpire === "---" ? "20" : group.ringExpire,
                            robKey: group.robKey === "---" ? "" : group.robKey,
                            enablePortMultiGroup: group.enablePortMultiGroup || "0",
                            ports: Array.from(
                              { length: PORT_GROUP_TOTAL_PORTS },
                              (_, i) => {
                                const portNum = i + 1;
                                const portsArr = group.ports
                                  .split(",")
                                  .map((p) => parseInt(p.trim(), 10));
                                return portsArr.includes(portNum);
                              }
                            ),
                          });
                          setPortsPage(1);
  ...
  const renderAddForm = () => {
    return (
      ...
      >
        ...
        >
          {form.id ? "Port Group-Modify" : PORT_GROUP_ADD_TITLE}
        </div>
        ...
        <div style={{ padding: "20px 24px", backgroundColor: C.pageBg }}>
          ...
          <div
            style={{
              background: "#fff",
              border: `1px solid ${validationErrors.ports ? C.errorRed : C.cardBorder}`,
              borderRadius: 6,
              padding: 16,
            }}
          >
            ...
            {validationErrors.ports && (
              <div style={{ color: C.errorRed, fontSize: 11, marginTop: 8 }}>
                {validationErrors.ports}
              </div>
            )}
          </div>
