...
import { Button, Dialog, DialogTitle, DialogContent, DialogActions, TextField, Select as MuiSelect, MenuItem, FormControl, Alert, CircularProgress } from "@mui/material";
...
const IPCallInOriCalleeID = () => {
  ...
  const handleSave = async () => {
    ...
    if (!formData.call_initiator) {
      errors.call_initiator = "Call Initiator is required.";
    }
    if (!formData.callerid_prefix) {
      errors.callerid_prefix = "CallerID Prefix is required.";
    }
    if (!formData.calleeid_prefix) {
      errors.calleeid_prefix = "CalleeID Prefix is required.";
    }
    ...
    try {
      ...
      if (editIndex !== null) {
        ...
        if (response.response) {
          ...
          showMessage("error", "Failed to update number manipulation");
        }
      } else {
        // Create new
        ...
        if (response.response) {
          ...
          showMessage("error", "Failed to create number manipulation");
        }
      }
      ...
      if (error.message === "Network Error") {
        ...
        showMessage("error", error.message || "Failed to save number manipulation");
      }
    } finally {
            ...
            {getUpdatedFields().map((field) => (
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  {field.type === "select" ? (
                    <>
                      <FormControl size="small" fullWidth error={!!validationErrors[field.name]}>
                        <MuiSelect
                          ...
                          sx={{
                            ...
                            "& .MuiOutlinedInput-notchedOutline": {
                              borderColor: !!validationErrors[field.name] ? C.errorRed : "#cbd5e1",
                            },
                            "&:hover .MuiOutlinedInput-notchedOutline": {
                              borderColor: !!validationErrors[field.name] ? C.errorRed : "#94a3b8",
                            },
                            "&.Mui-focused .MuiOutlinedInput-notchedOutline": {
                              borderColor: !!validationErrors[field.name] ? C.errorRed : "#1e2d42",
                            },
                          }}
                        >
                      ...
                      <TextField
                        ...
                        sx={{
                          "& .MuiOutlinedInput-root": {
                            borderRadius: 1,
                            fontSize: 13,
                            "& fieldset": { borderColor: !!validationErrors[field.name] ? C.errorRed : "#cbd5e1" },
                            "& hover fieldset": { borderColor: !!validationErrors[field.name] ? C.errorRed : "#94a3b8" },
                            "&.Mui-focused fieldset": { borderColor: !!validationErrors[field.name] ? C.errorRed : "#1e2d42" },
                          },
                        }}
                      />
                      {validationErrors[field.name] && (
