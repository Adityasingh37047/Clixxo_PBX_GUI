...
const RouteIpPstnPage = () => {
  ...
  const handleSave = async () => {
    ...
    if (!formData.callSource) {
      errors.callSource = "Call Source is required.";
    }
    if (!formData.callerIdPrefix) {
      errors.callerIdPrefix = "CallerID Prefix is required.";
    } else if (!validatePrefix(formData.callerIdPrefix)) {
      errors.callerIdPrefix = "Invalid CallerID Prefix! Only numbers (0-9) and asterisks (*) are allowed.";
    }
    if (!formData.calleeIdPrefix) {
      errors.calleeIdPrefix = "CalleeID Prefix is required.";
    } else if (!validatePrefix(formData.calleeIdPrefix)) {
      errors.calleeIdPrefix = "Invalid CalleeID Prefix! Only numbers (0-9) and asterisks (*) are allowed.";
    }
    if (!formData.callDestination) {
      errors.callDestination = "Call Destination is required.";
    }
  ...
  const allPageSelected =
    pagedIndices.length > 0 && pagedIndices.every((idx) => selected.includes(idx));
  const somePageSelected =
    pagedIndices.length > 0 &&
    pagedIndices.some((idx) => selected.includes(idx)) &&
    !allPageSelected;
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (allSuccessful) {
        ...
        showMessage("error", "Some routes could not be deleted. Please try again.");
      }
    } catch (error) {
          ...
          >
            {ROUTE_IP_PSTN_FIELDS.map((field) => (
              <FieldRow key={field.key} label={`${field.label}:`}>
                {field.type === "select" ? (
                  <>
                    <FormControl size="small" fullWidth error={!!validationErrors[field.key]}>
                      <Select
                        ...
                        sx={{
                          ...
                          "& .MuiOutlinedInput-notchedOutline": {
                            borderColor: !!validationErrors[field.key] ? C.errorRed : "#cbd5e1",
                          },
                          "&:hover .MuiOutlinedInput-notchedOutline": {
                            borderColor: !!validationErrors[field.key] ? C.errorRed : "#94a3b8",
                          },
                          "&.Mui-focused .MuiOutlinedInput-notchedOutline": {
                            borderColor: !!validationErrors[field.key] ? C.errorRed : "#1e2d42",
                          },
                        }}
                      >
                    ...
                    <TextField
                      ...
                      sx={{
                        "& .MuiOutlinedInput-root": {
                          "& fieldset": { borderColor: !!validationErrors[field.key] ? C.errorRed : "#cbd5e1" },
                          "&:hover fieldset": { borderColor: !!validationErrors[field.key] ? C.errorRed : "#94a3b8" },
                          "&.Mui-focused fieldset": { borderColor: !!validationErrors[field.key] ? C.errorRed : "#1e2d42" },
                        },
                      }}
                    />
                    {validationErrors[field.key] && (
