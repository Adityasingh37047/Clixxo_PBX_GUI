...
import { saveCallerBlacklist, saveCalleeBlacklist, fetchAllNumberFilters, fetchNumberFilters, deleteNumberFilter, deleteAllNumberFilters } from '../api/apiService';
...
const Blacklist = () => {
  ...
  const fetchBlacklistData = async () => {
    ...
    try {
      ...
      displayToast('Failed to load blacklist data. Please refresh the page.', 'error');
    } finally {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (modalType === 'caller') {
        ...
        await saveCallerBlacklist(apiData);
        ...
        await saveCalleeBlacklist(apiData);
      ...
      if (isEditMode) {
        ...
        try {
          const verifyResp = await fetchNumberFilters('blacklist', modalData.idValue);
          ...
          if (existsInTarget) {
            await deleteNumberFilter('blacklist', originalIdValue, subtype, originalGroupNo);
          } else {
      ...
      const errorMessage = error.response?.data?.message || error.message || 'Failed to save blacklist. Please try again.';
  ...
  const handleCallerSearch = async () => {
    ...
    try {
      const response = await fetchNumberFilters('blacklist', callerSearch.trim());
  ...
  const handleCalleeSearch = async () => {
    ...
    try {
      const response = await fetchNumberFilters('blacklist', calleeSearch.trim());
  ...
  const handleCallerDelete = async () => {
    ...
    try {
      ...
      for (const idx of callerChecked) {
        ...
        const response = await deleteNumberFilter('blacklist', item.callerId, 'callerid', item.groupNo);
        if (!response.success) {
  ...
  const handleCalleeDelete = async () => {
    ...
    try {
      ...
      for (const idx of calleeChecked) {
        ...
        const response = await deleteNumberFilter('blacklist', item.calleeId, 'calleeid', item.groupNo);
        if (!response.success) {
  ...
  return (
    <div className="bg-gray-50 min-h-[calc(100vh-80px)] p-2 sm:p-4 md:p-6">
      {isInitialLoading && (
        <div className="flex justify-center items-center h-64">
          <div className="flex flex-col items-center gap-4">
            ...
            <span className="text-gray-600 text-sm sm:text-base">Loading blacklist data...</span>
          </div>
        </div>
      )}
          ...
          <div className="flex flex-col xl:flex-row gap-4 sm:gap-6 w-full mx-auto">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="bg-gray-200 w-full flex flex-col">
                <div className="w-full bg-gradient-to-b from-[#b3e0ff] to-[#3d92d0] text-[#222] font-semibold text-lg text-center py-1">CallerID Blacklist</div>
                  ...
                  <table className="w-full min-w-[500px] border-collapse table-auto">
                    <thead>
                      <tr>
                        ...
                        <th className="bg-white text-gray-800 font-semibold text-xs border border-gray-400 whitespace-nowrap text-center" style={{ padding: '6px 8px', height: 32,  }}>Group No.</th>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="bg-gray-200 w-full flex flex-col">
                <div className="w-full bg-gradient-to-b from-[#b3e0ff] to-[#3d92d0] text-[#222] font-semibold text-lg text-center py-1">CalleeID Blacklist</div>
                <div className="overflow-x-auto w-full border-l-2 border-r-2 border-b-2 border-gray-400" style={{ height: '360px', maxHeight: '360px', overflowY: 'auto' }}>
                  <table className="w-full min-w-[500px] border-collapse table-auto">
                    <thead>
                      <tr>
                    ...
                    <tbody>
                      {calleeRows.map((row, idx) => (
                        <tr key={idx} style={{ borderBottom: '1px solid #bbb', height: 32 }}>
                          ...
                          <td className="border border-gray-400 text-center bg-white text-xs" style={{ padding: '6px 8px', height: 32 }}>{row.groupNo}</td>
                          {/* <td className="border border-gray-400 text-center bg-white text-xs" style={{ padding: '6px 8px', height: 32 }}>{row.noInGroup}</td> */}
      ...
      >
        <DialogTitle className="bg-gray-600 text-white text-center font-semibold text-lg">{modalType === 'caller' ? 'CallerIDs in Blacklist' : 'CalleeIDs in Blacklist'}</DialogTitle>
