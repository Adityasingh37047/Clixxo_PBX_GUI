...
const SELECT_MENU_PROPS = {
  PaperProps: { sx: { maxHeight: 260, maxWidth: "90vw" } },
};
...
const InboundRoutesPage = () => {
  ...
  // Selection (index-based, original logic)
  ...
  // Page-level select all for header checkbox
  ...
  const handleSave = async () => {
    ...
    if (destination === "Extension_Range") {
      ...
      if (!/^\d+\s*-\s*\d+$/.test(extensionRange.trim())) {
    ...
    const apiPayload = {
      ...
      enable_t38: toApiYesNo(enableT38, "no"),
      enable_time_condition: toApiYesNo(enableTimeCondition, "no"),
      dest_type: destType,
      dest_value: destValue || "",
      member_trunks: Array.isArray(selectedTrunks) ? selectedTrunks : [],
      enabled: toApiYesNo(enabled, "yes"),
      priority: parsedPriority,
      enable_mobility_ext: toApiYesNo(enableMobilityExtension, "no"),
