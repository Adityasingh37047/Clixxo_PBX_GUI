...
const PbxMonitor = () => {
  ...
  const getExtensionStatusLabel = (row) => {
    const status = normalizeStatus(
      row?.status || row?.registration_status || row?.register_status,
    );
    ...
    return {
      text: row?.status || row?.registration_status || "Unknown",
      tone: "neutral",
    };
  };
  ...
  const getTrunkStatusLabel = (row) => {
    ...
    if (status === "rejected") return { text: "Rejected", tone: "bad" };
    return {
      text: row?.trunk_status || row?.status || "Unknown",
      tone: "neutral",
    };
  };
  ...
  return (
    <div
      className="w-full min-h-[calc(100vh-200px)] flex flex-col items-center box-border md:p-2"
      style={{ backgroundColor: "#dde0e4" }}
    >
      <div
        className="w-full max-w-[1320px] mx-auto mt-0 rounded-t-lg"
        style={{
          background: "linear-gradient( #3E5475 100%)",
          boxShadow: "0 2px 8px 0 rgba(80,160,255,0.10)",
        }}
      >
        <div className="rounded-t-lg h-8 flex items-center justify-center font-semibold text-[18px] text-[#ffffff] shadow-sm">
          PBX Monitor
        </div>
      </div>
      ...
      <div className="w-full max-w-[1320px] mx-auto p-0">
        <div className="bg-[#f8fafd] border border-gray-300 border-t-0 rounded-none p-0">
          <Tabs
            ...
            textColor="inherit"
            TabIndicatorProps={{
              style: { backgroundColor: "#3E5475", height: 3 },
            }}
            sx={{
              "& .MuiTab-root": {
                minHeight: 28,
          ...
          >
            <Tab
              label="EXTENSION"
              value="extension"
              sx={{ fontSize: 12, fontWeight: 700, minHeight: 28 }}
            />
            <Tab
              label="TRUNK"
              value="trunk"
        ...
        <div className="flex items-center bg-[#f8fafd] border border-gray-300 border-t-0 border-b-0 px-3 py-0.5 rounded-none">
          <div className="text-[11px] text-gray-700 whitespace-nowrap">
            ...
            {activeTab === "extension" ? "(Extension)" : "(Trunk)"}
          </div>
        </div>
        ...
        <div className="bg-[#f8fafd] border border-gray-300 border-t-0 rounded-b-lg shadow-sm overflow-x-auto">
          {activeTab === "extension" ? (
            <table className="w-full table-fixed min-w-[1200px] bg-[#f8fafd] border border-gray-300">
              <thead>
                <tr>
                  <th className="bg-white text-gray-800 font-semibold text-[11px] border border-gray-300 px-2 py-1 whitespace-nowrap w-[120px]">
                    Status
                  </th>
                  <th className="bg-white text-gray-800 font-semibold text-[11px] border border-gray-300 px-2 py-1 whitespace-nowrap w-[120px]">
                    Extension
                  </th>
                  <th className="bg-white text-gray-800 font-semibold text-[11px] border border-gray-300 px-2 py-1 whitespace-nowrap w-[260px]">
                    Name
                  </th>
                  <th className="bg-white text-gray-800 font-semibold text-[11px] border border-gray-300 px-2 py-1 whitespace-nowrap w-[90px]">
                    Type
                  </th>
                  <th className="bg-white text-gray-800 font-semibold text-[11px] border border-gray-300 px-2 py-1 whitespace-nowrap w-[300px]">
                    IP and Port
                  </th>
                </tr>
              </thead>
                  ...
                  extensionRows.map((row) => {
                    ...
                    return (
                      <tr key={row.id}>
                        ...
                        >
                          <span
                            className={`px-1.5 py-0.5 rounded text-[11px] font-medium ${getBadgeClass(status.tone)}`}
                          >
                            {status.text}
                          </span>
                        </td>
                        ...
                        <td className="border border-gray-300 px-2 py-1 text-center whitespace-nowrap overflow-hidden text-ellipsis">
                          {row.name || "-"}
                        </td>
                        <td className="border border-gray-300 px-2 py-1 text-center whitespace-nowrap">
            ...
            <table className="w-full table-fixed min-w-[1200px] bg-[#f8fafd] border border-gray-300">
              <thead>
                <tr>
                  <th className="bg-white text-gray-800 font-semibold text-[11px] border border-gray-300 px-2 py-1 whitespace-nowrap w-[160px]">
                    Trunk Status
                  </th>
                  <th className="bg-white text-gray-800 font-semibold text-[11px] border border-gray-300 px-2 py-1 whitespace-nowrap w-[280px]">
                    Trunk Name
                  </th>
                  <th className="bg-white text-gray-800 font-semibold text-[11px] border border-gray-300 px-2 py-1 whitespace-nowrap w-[90px]">
                    Type
