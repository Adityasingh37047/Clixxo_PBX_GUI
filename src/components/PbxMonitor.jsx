...
const StatusBadge = ({ tone, text }) => {
  const colors = {
    ok: { color: C.successGreen, dot: C.successGreen }, // registered background color bg: "#f0fdf4",
    bad: { color: C.errorRed, dot: C.errorRed }, // unregistered background color bg: "#fef2f2",
...
const PbxMonitor = () => {
  ...
  const getTrunkStatus = (row) => {
    const s = normalizeStatus(
      row?.trunk_status || row?.status || row?.registration_status,
    );
    if (s === "registered") return { text: "Registered", tone: "ok" };
    if (s === "unregistered") return { text: "Unregistered", tone: "bad" };
    if (s === "rejected") return { text: "Rejected", tone: "bad" };
  ...
  const extUnregistered = extensionRows.filter(
    (r) => normalizeStatus(r.status) !== "registered",
  ).length;
  ...
  const totalRows =
    activeTab === "extension" ? extensionRows.length : trunkRows.length;
      ...
      <div style={{ maxWidth: "100%", margin: "0 auto" }}>
        ...
        >
          ...
          >
            ...
            >
              Total: {totalRows}{" "}
              {activeTab === "extension" ? "Extensions" : "Trunks"}
            </span>
                    ...
                    filteredExtensions.map((row, idx) => {
                      ...
                      return (
                        <tr
                          ...
                          style={{
                            background: idx % 2 === 1 ? "#f3f4f6" : "#ffffff",
                        ...
                        >
                          ...
                          <TD>{row.name || null}</TD>
                          <TD align="center">
                            <TypePill text={getExtensionType(row)} />
                          </TD>
