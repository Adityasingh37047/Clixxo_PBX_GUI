...
const Paging = () => {
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(rows.length / PAGING_ITEMS_PER_PAGE),
  );
  const pagedRows = rows.slice(
    (page - 1) * PAGING_ITEMS_PER_PAGE,
    page * PAGING_ITEMS_PER_PAGE,
  );
  ...
  useEffect(() => {
    setPage((current) =>
      Math.min(
        Math.max(1, current),
        Math.max(1, Math.ceil(rows.length / PAGING_ITEMS_PER_PAGE)),
      ),
    );
  }, [rows]);
            ...
            <tbody>
              {rows.length === 0 ? (
                <tr>
                  ...
                  >
                    No paging entries yet. Click &quot;Add New&quot; to create
                    one.
                  </td>
                </tr>
              ) : (
                ...
                <div className="mt-3">
                  ...
                  <div className="grid grid-cols-[1fr_48px_1fr_48px] gap-3 items-start">
                    ...
                    <div className="flex flex-col gap-1 pt-7">
                      <button
                        type="button"
                        className="h-8 border border-gray-500 bg-[#d9dde3] text-sm font-semibold hover:bg-[#c5cbd3] flex items-center justify-center"
