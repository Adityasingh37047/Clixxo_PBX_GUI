      ...
      <div className="w-full" style={{ maxWidth: 1000 }}>
        ...
        >
          ...
          >
            ...
            <div style={{ display: "flex", gap: "8px" }}>
              ...
              {mode === null && (
                <button
                  onClick={() => { if (!canWrite) { showReadOnlyToast(); return; } openAdd(); }}
                  style={{
                  ...
                  onMouseEnter={(e) => {
                    if (canWrite) e.currentTarget.style.background = "linear-gradient(to bottom, #3E5475 0%, #5A6F8F 100%)";
                  }}
                  onMouseLeave={(e) => {
                    if (canWrite) e.currentTarget.style.background = "linear-gradient(to bottom, #5A6F8F 0%, #3E5475 60%, #2C3E57 100%)";
                  }}
                >
                  + Add User
                </button>
                          ...
                          >
                            {!isSuperAdmin && (
                              <EditDocumentIcon
                                ...
                                titleAccess="Edit"
                                onClick={() => { if (!canWrite) { showReadOnlyToast(); return; } openEdit(user); }}
                                style={{
                            ...
                            {!isSuperAdmin && (
                              <DeleteOutlineOutlinedIcon
                                className="transition-opacity"
                                titleAccess="Delete"
                                onClick={() => { if (!canWrite) { showReadOnlyToast(); return; } handleDelete(user); }}
                                style={{
                                  cursor: canWrite ? "pointer" : "not-allowed",
                                  opacity: canWrite ? 0.7 : 0.3,
                                  color: "#dc2626",
        ...
        {mode && (
          <>
            ...
            >
              <button
                onClick={() => { if (!canWrite) { showReadOnlyToast(); return; } handleSave(); }}
                disabled={saving}
                style={{
                  background: "linear-gradient(to bottom, #5A6F8F 0%, #3E5475 60%, #2C3E57 100%)",
                  color: "#fff",
                ...
                onMouseEnter={(e) => {
                  if (!saving && canWrite)
                    e.currentTarget.style.background = "linear-gradient(to bottom, #3E5475 0%, #5A6F8F 100%)";
                }}
                onMouseLeave={(e) => {
                  if (!saving && canWrite)
                    e.currentTarget.style.background = "linear-gradient(to bottom, #5A6F8F 0%, #3E5475 60%, #2C3E57 100%)";
                }}
              >
                {saving ? "Saving..." : "Save"}
              </button>
