...
import { fetchCallQueueActivity, fetchCallQueueAgentStats, fetchCallQueueQueueStats } from '../api/apiService';
...
const AnsweredRateBar = ({ rate }) => (
  <div style={{ width: '100%', background: '#e2e8f0', borderRadius: 3, height: 6, overflow: 'hidden' }}>
    <div style={{
...
const StatCard = ({ label, value, color }) => (
  <div style={{
    ...
    <div style={{ fontSize: 22, fontWeight: 700, color: color || C.accent, marginBottom: 4 }}>
      {value ?? 0}
    </div>
    <div style={{ fontSize: 11, color: C.labelText, fontWeight: 500 }}>{label}</div>
  </div>
);
...
const LoadingRow = ({ cols }) => (
  <tr>
    <td colSpan={cols} style={{ textAlign: 'center', padding: 32, color: C.mutedText, fontSize: 13 }}>
...
const EmptyRow = ({ cols, msg = 'No data available' }) => (
  <tr>
    <td colSpan={cols} style={{ textAlign: 'center', padding: '32px 0', color: C.mutedText, fontSize: 13 }}>
      {msg}
    </td>
  </tr>
);
...
const ActionBtn = ({ children, onClick, variant = 'dark' }) => {
  const styles = {
    dark:     { background: C.cardHeader, color: '#fff', border: `1px solid #162233` },
...
const CallQueueStatistics = ({ onBack, initialQueue }) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try { const d = await fetchCallQueueAgentStats(selectedQueue); setAgentData(Array.isArray(d) ? d : []); } catch { /**/ }
      finally { setLoadingAgent(false); }
      try { const d = await fetchCallQueueQueueStats(selectedQueue); setQueueData(Array.isArray(d) ? d : []); } catch { /**/ }
  ...
  useEffect(() => { setAgentSearch(''); }, [activeTab]);

  const filteredAgents = agentSearch.trim()
    ? agentData.filter(r =>
        String(r.agent_number ?? r.agentNumber ?? '').includes(agentSearch.trim()) ||
        String(r.agent_name ?? r.agentName ?? '').toLowerCase().includes(agentSearch.toLowerCase())
      )
    : agentData;
      ...
      <div style={{ maxWidth: '100%', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          <div style={{ fontSize: 11, color: C.mutedText }}>
            ...
            <span style={{ color: C.valueText, fontWeight: 600 }}>Call Queue Statistics</span>
          </div>
          {lastUpdated && (
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <ActionBtn
                variant="outline"
                onClick={() => { setAgentData([]); setQueueData([]); }}
              >
                Clear
              </ActionBtn>
              <ActionBtn variant="accent" onClick={onBack}>← Back</ActionBtn>
            </div>
          </div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 900 }}>
                  ...
                  <tbody>
                    {loadingAgent && agentData.length === 0 ? (
                      ...
                      <EmptyRow
                        cols={10}
                        msg={agentSearch ? `No results for "${agentSearch}"` : 'No agent data available'}
                      />
                      ...
                      >
                        <TD align="center">
                          <strong style={{ color: C.valueText }}>{row.agent_number ?? row.agentNumber ?? '—'}</strong>
                        </TD>
                        <TD>{row.agent_name ?? row.agentName ?? null}</TD>
                        <TD align="center" mono>{row.online_time ?? row.onlineTime ?? null}</TD>
                        <TD align="center">{row.total_calls ?? row.totalCalls ?? 0}</TD>
                        <TD align="center">{row.answered_calls ?? row.answeredCalls ?? 0}</TD>
                        <TD align="center">
                          <RatePill value={row.answered_rate ?? row.answeredRate ?? 0} />
                        </TD>
                        <TD align="center">{row.caller_hangup_while_agent_ring ?? row.callerHangup ?? 0}</TD>
                        <TD align="center" mono>{row.avg_talk_time ?? row.averageTalkTime ?? null}</TD>
                        <TD align="center">{row.idle_time ?? row.idleTime ?? null}</TD>
                        <TD align="center">{row.avg_idle_time ?? row.averageIdleTime ?? null}</TD>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 800 }}>
                  ...
                  <tbody>
                    {loadingQueue && queueData.length === 0 ? (
                      ...
                      >
                        <TD align="center">
                          <strong style={{ color: C.valueText }}>{row.queue_number ?? row.queueNumber ?? '—'}</strong>
                        </TD>
                        ...
                        <TD align="center" mono>{row.average_wait_time ?? row.avgWaitTime ?? null}</TD>
                        <TD align="center" mono>{row.average_talk_time ?? row.avgTalkTime ?? null}</TD>
                        <TD align="center">{row.caller_hangup ?? row.callerHangup ?? 0}</TD>
                        <TD align="center">{row.call_queue_timeout_calls ?? row.timeoutCalls ?? 0}</TD>
                        <TD align="center">{row.callback_calls ?? row.callbackCalls ?? 0}</TD>
                      </tr>
                    ))}
                  </tbody>
                </table>
...
const ActiveCallQueue = () => {
  ...
  const loadActivity = useCallback(async (isInitial = false) => {
    ...
    try {
      ...
      setSelectedQueue(prev => {
        if (prev) {
          return list.find(q =>
            (q.queue_number ?? q.number) === (prev.queue_number ?? prev.number)
          ) || list[0] || null;
        }
        return list[0] || null;
      });
  ...
  if (showStats) {
    const qNum = selectedQueue
      ...
      : '';
    return <CallQueueStatistics onBack={() => setShowStats(false)} initialQueue={qNum} />;
  }
  ...
  const norm = (q) => ({
    ...
    avgWaitTime:  q.average_waiting_time ?? q.avg_wait_time ?? q.avgWaitTime ?? '0:00:00',
    avgTalkTime:  q.average_talking_time ?? q.avg_talk_time ?? q.avgTalkTime ?? '0:00:00',
      ...
      <div style={{ maxWidth: '100%', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          <div style={{ fontSize: 11, color: C.mutedText }}>
            ...
            <span style={{ color: C.valueText, fontWeight: 600 }}>Active Call Queue</span>
          </div>
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      boxShadow: isSelected ? `0 0 0 1px ${C.accent}20` : '0 1px 3px rgba(0,0,0,0.04)',
                    }}
                    onMouseEnter={e => { if (!isSelected) e.currentTarget.style.background = '#f8fafc'; }}
                    onMouseLeave={e => { if (!isSelected) e.currentTarget.style.background = C.cardBg; }}
                  >
                    <div style={{ fontSize: 13, fontWeight: 700, color: C.accent, marginBottom: 4 }}>
                      {n.number}
                    </div>
                    <div style={{ fontSize: 11, color: C.mutedText, marginBottom: 8 }}>{n.name}</div>
                    ...
                    <span style={{
                      ...
                      <span style={{ width: 5, height: 5, borderRadius: '50%', background: C.successGreen }} />
                      {n.status}
                    </span>

                    <AnsweredRateBar rate={n.answeredRate} />
                    <div style={{ fontSize: 10, color: C.mutedText, marginTop: 4 }}>
            ...
            {sel && (
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                <div style={{
                  ...
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                      <span style={{ fontSize: 18, fontWeight: 700, color: C.valueText }}>
                        {sel.number}
                      </span>
                      <span style={{ fontSize: 13, color: C.labelText }}>({sel.name})</span>
                      <span style={{
                        display: 'inline-flex',
                        alignItems: 'center',
                        gap: 4,
                        ...
                        <span style={{ width: 6, height: 6, borderRadius: '50%', background: C.successGreen }} />
                        {sel.status}
                      </span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <span style={{ fontSize: 11, color: C.labelText }}>Answered Rate</span>
                      <div style={{ width: 160 }}>
                        <AnsweredRateBar rate={sel.answeredRate} />
                      </div>
                      <span style={{ fontSize: 13, fontWeight: 700, color: C.valueText }}>
                        {sel.answeredRate}%
                      </span>
                    </div>
                  </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10 }}>
                  <StatCard label="Total Calls"     value={sel.totalCalls}    color={C.accent}        />
                  <StatCard label="Answered Calls"  value={sel.answeredCalls} color={C.successGreen}  />
                  <StatCard label="Waiting Calls"   value={sel.waitingCalls}  color={C.amber}         />
                  <StatCard label="Abandoned Calls" value={sel.abandonedCalls} color={C.errorRed}     />
                </div>

                {/* Agent metrics — 4 cards */}
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10 }}>
                  <StatCard label="Total Agents"  value={sel.totalAgents}  color={C.purple}      />
                  <StatCard label="Active Agents" value={sel.activeAgents} color={C.teal}         />
                  <StatCard label="Idle Agents"   value={sel.idleAgents}   color={C.amber}        />
                  <StatCard label="On Call Agents" value={sel.onCallAgents} color={C.darkGreen}   />
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 10 }}>
                  {[
                    ...
                    <div key={label} style={{
                      ...
                      <span style={{ fontSize: 12, color: C.labelText, fontWeight: 500 }}>{label}</span>
                      <span style={{ fontSize: 15, fontWeight: 700, color: C.accent, fontFamily: 'monospace' }}>
                        {value}
                      </span>
                    </div>
                  ))}
